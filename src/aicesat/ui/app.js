...
//
// Match the AUTH HELP TEXT, not the word "token". The old pattern included /\btoken\b/, and the message for an
// area with no index says "...(check your selection and the token)" — so a coverage failure was reported as an
// authentication failure. That cost real debugging time: the token was valid for another 11 days and the actual
// cause was that no collection had an index over the selection. A banner that names the wrong subsystem is worse
// than a generic one, so these patterns are phrases only auth.py's _AUTH_HELP produces.
AICESAT.showError = (msg) => {
  ...
  const auth = /No working Earthdata Login|EARTHDATA_TOKEN|AICESAT_EDL_FILE|\bnetrc\b|urs\.earthdata\.nasa\.gov/i.test(s);
