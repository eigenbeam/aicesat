...
AICESAT.ready.then(api => {
  ...
  // ---- MCP App specifics
  if (api.kind === 'app') {
    $('fullscreenBtn').hidden = false; $('fullscreenBtn').onclick = () => api.fullscreen();
    ...
    AICESAT.applyHostContext = ctx => { if (ctx && ctx.theme) document.documentElement.dataset.theme = ctx.theme; };
  }
