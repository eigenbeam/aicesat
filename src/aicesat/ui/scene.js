...
async function loadScene() {
  ...
  fitView(); render(); updateLabels(); updateStats(); initTimeSeries();
...
const H3_EDGE_M = {8: 461, 9: 174, 10: 66, 11: 25};
...
function initTimeSeries() {
  ...
  tsLabels(); renderCandList(); $('tsChart').hidden = true; $('tsReadout').textContent = ''; $('tsCaveat').hidden = true; $('tsStatus').textContent = '';
}
async function findCandidates() {
  ...
  try {
    ...
    if (candidates.length) selectCand(0); else { $('tsChart').hidden = true; $('tsReadout').textContent = 'no cells with 3+ time windows — try a larger cell size or a wider time window'; }
...
function renderCandList() {
  $('tsList').innerHTML = candidates.map((c, i) =>
    '<div class="tscand ' + (i === candSel ? 'on' : '') + '" data-i="' + i + '"><b>' + c.n_bins + ' epochs</b> · ' + c.span_years + ' yr · ' + c.slope_deg + '° slope <span class="small">' + c.n_points + ' pts</span></div>').join('');
  $('tsList').querySelectorAll('.tscand').forEach(el => el.onclick = () => selectCand(+el.dataset.i));
}
function selectCand(i) { candSel = i; renderCandList(); drawChart(); render(); }
...
function candidateLayers() {
  ...
  return [new deck.PolygonLayer({id: 'cands', data: rings, getPolygon: d => d.poly, filled: true, stroked: true, pickable: true,
    getFillColor: d => d.sel ? [255, 214, 110, 70] : [235, 235, 245, 10],
    getLineColor: d => d.sel ? [255, 214, 110, 255] : [205, 205, 220, 110],
    lineWidthUnits: 'pixels', getLineWidth: d => d.sel ? 3 : 1.4, lineWidthMinPixels: 1,
