...
const MISSION_COLORS = {GLAS: [240, 228, 66], ICESSN: [40, 200, 120], ATL06: [40, 140, 225], ICESAT2: [213, 94, 0]};
...
function updateLabels() {
  ...
  // Satellite-imagery toggle: only enabled once the area's imagery has been fetched (prefetched during the build)
...
async function pollUntilReady() {
  const myId = sceneId;
  let status = 'loading';
  try { const list = await api.scenes(); const rec = (list || []).find(s => s.scene_id === myId); status = rec ? rec.status : 'loading'; }
  catch (e) {}
...
function candidateLayers() {
  ...
  const layers = [new deck.PolygonLayer({id: 'cands', data: rings, getPolygon: d => d.poly, filled: true, stroked: true, pickable: true,
    getFillColor: d => d.sel ? [120, 225, 255, 60] : [200, 214, 245, 32],   // brighter than before so unselected cells read clearly
  ...
  if (c && bounds) {
    ...
    const stickH = Math.max(bounds.maxz - bounds.minz, 40) * Z_EXAG * 0.9;
    ...
    // Leader line from the hex up to the floating label. depthTest off so the tether is ALWAYS visible (never buried
    // in the dense point cloud); a dark halo behind it keeps it legible over any colour, and an anchor dot sits right
    // on the hex centre so the connection reads unambiguously.
    const stickPath = [[base, top]];
    layers.push(new PathLayer({id: 'cand-stick-halo', data: stickPath, getPath: d => d, getColor: [8, 14, 22, 190],
      getWidth: 6, widthUnits: 'pixels', parameters: {depthTest: false}, updateTriggers: {getPath: Z_EXAG}}));
    layers.push(new PathLayer({id: 'cand-stick', data: stickPath, getPath: d => d, getColor: MARK, getWidth: 2.5,
      widthUnits: 'pixels', parameters: {depthTest: false}, updateTriggers: {getPath: Z_EXAG}}));
    layers.push(new PointCloudLayer({id: 'cand-stick-base', data: [0], getPosition: () => base, getColor: MARK,
      pointSize: 5, sizeUnits: 'pixels', updateTriggers: {getPosition: Z_EXAG}}));
    layers.push(new PointCloudLayer({id: 'cand-stick-top', data: [0], getPosition: () => top, getColor: MARK,
      pointSize: 7, sizeUnits: 'pixels', updateTriggers: {getPosition: Z_EXAG}}));
    layers.push(new TextLayer({id: 'cand-stick-label', data: [{position: top}], getPosition: d => d.position,
      ...
      getPixelOffset: [0, -10], billboard: true, fontFamily: 'ui-sans-serif, system-ui, sans-serif', characterSet: 'auto',
