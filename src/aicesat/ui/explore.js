...
AICESAT.ExploreView = class {
  constructor(root, api, openScene) {
    ...
    $('exBox').onclick = () => { this.map.setMode('box'); $('exBox').classList.add('on'); $('exPoly').classList.remove('on'); };
    $('exPoly').onclick = () => { this.map.setMode('poly'); $('exPoly').classList.add('on'); $('exBox').classList.remove('on'); };
