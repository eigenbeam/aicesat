...
AICESAT.MapView = class {
  ...
  // A hexagonal patch of cells around the point the globe is facing. gridDisk (not a lon/lat rectangle) so it works
  // uniformly at ANY latitude — including over the pole, where a lat/lon box degenerates and the grid used to vanish.
  gridCells() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (cells.length > 5000 && res > 0) { res--; continue; }   // safety: too many -> coarser resolution
      return {cells, res};
    }
    return {cells: [], res};
  }
