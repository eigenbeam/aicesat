...
def main():
    ...
    granules = coverage.search(coverage.GLAS_SHORT_NAME, coverage.GLAS_VERSION, bbox, None, polygon=ring)
    names = {coverage.granule_name(g): g for g in granules}
    md0 = index_glas._index_dir(res); md0.mkdir(parents=True, exist_ok=True)
